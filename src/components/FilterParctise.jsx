...
const FilterPractice = () => {
  ...
  const submit = (event) => {
    ...
    const newData = [...data];
    newData.push({ name, lastName, email });
    setData(newData);
    setName("");
    setLastName("");
    setEmail("");
...
const filteredData = data.filter((item) => item.name.includes(search)||item.lastName.includes(search)||item.email.includes(search));
    return (
